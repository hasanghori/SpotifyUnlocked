...
class PlaylistCreation:
    ...
    def createVibe(self, playlistName, token):
        #find playlist
        spotifyObject = Spotify(auth=token)
        spotifyUser = spotifyObject.current_user()
        username = spotifyUser['id']
            ...
            for i in range(0, len(playlistSongs['items'])):
                try:
                    ...
                    songObj = [songFeatures[0]['acousticness'],
                               ...
                               round(songFeatures[0]['tempo'] / 200, 6), songFeatures[0]['valence']]
                    songObjectsList.append(songObj)
                except:
    ...
    def likedSongsCreateVibe(self, token):
        ...
        # send it to kmeans and get centroidsAndError
        # print(genreDict)
        #self.testGenres(genreDict=genreDict) USED TO BE IMPORTANT NOW TEST
        centroidsAndError = self.KMeansVibe(data=songObjectsList)
    ...
    def testGenres(self, genreDict):
        ...
        superGenreDict = {}
        genreGrouping = {}
        superGenreDictAndList = {} #this will store all the subgenres of the main genre
        numGenres = int(0)
        for key in genreDict:
            ...
            if superGenre not in superGenreDictAndList.keys():
                superGenreDictAndList[superGenre] = [key]
            else:
                superGenreDictAndList[superGenre].append(key)
        ...
        for key in superGenreDict:
            if superGenreDict[key] / numGenres < 0.05:
                genresToTest.append(random.choice(superGenreDictAndList[key]))


        return genresToTest

    def refineGenreVibe(self, genreDict, removeGenre):


        myDict = {key: val for key, val in genreDict.items() if self.genreDictionary(val) != removeGenre}
    ...
    def findSongsToTest(self, genreList, token):
        # find user
        spotifyObject = Spotify(auth=token)
        spotifyUser = spotifyObject.current_user()
        username = spotifyUser['id']
        ...
        for genre in genreList:
            ...
            song = spotifyObject.search(q=query, limit=1, type='track',
                                 offset=random.randint(0, 10))
            #print(song[0]['items']['id'])
            superGenre = self.genreDictionary(genre.upper())
            try:
                songs[song['tracks']['items'][0]['id']] = superGenre
            except:
                print(genre)
                print("This genre does not have enough songs in it to search ^^")
            print(songs)
        return songs
    ...
    #def likedSongsShuffle():
    #def likedSongsPlaylistYouGrabSongsFrom():
    #def likeSongsPlaylistYouPutSongsIn():
    ...
    def recommendation(self, token, genresList):
        spotifyObject = Spotify(auth=token)
        songWorks = False
        songId = ''
        while (songWorks == False):
            try:
                #genresList = list(genreDict.keys())
                genre = random.choice(genresList)
                query = 'genre:' + str(genre)
                song = spotifyObject.search(q=query, limit=1, type='track',
                                    offset=random.randint(0, 10))
